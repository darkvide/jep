...
//cron.schedule(configurarCron, () => {
    ...
    const sqlServerClients = new Promise((resolve, reject) => {
        ...
        const idProductosPromise = Promise.all([sqlServerPromise, sqlServerProducts, sqlServerClients]).then(results => {
                ...
                return connection.query('SELECT twofowg1_jepweb.ps_product.id_product,twofowg1_jepweb.ps_product.supplier_reference FROM twofowg1_jepweb.ps_product where twofowg1_jepweb.ps_product.supplier_reference in ('+codart+');' , (error_prod, result_prod) => {
                    if (!error_prod) {
                        for (var i in result_prod) {
                            //id_productos.push("'"+String(result_prod[i].id_product).trim()+"'");
            ...
            /*
            Precios// no le pares bola a eso que se desconecto des pues de un tiempo de no hacer nada con la base
 ...
 /*
        ...
        /*
            ...
            */
            ...
            /*for(var k=0;k<id_productos.length;k++){
                connection.query('Select twofowg1_jepweb.ps_specific_price.id_product,twofowg1_jepweb.ps_specific_price.id_group from twofowg1_jepweb.ps_specific_price WHERE twofowg1_jepweb.ps_specific_price.id_product = ' + id_productos[k], (error_precio, result_precio) => {
                    if (!error_precio) {
                        if(result_precio.length > 1){
                            for (var l in result_precio) {
                                //aqui los updates
                                connection.query('UPDATE twofowg1_jepweb.ps_product SET ? WHERE twofowg1_jepweb.ps_product.id_product = ' + String(id_productos[k]).trim(), data, (error1, result1) => {
                                    if (!error1) {
                                        console.log(result1);
                                    } else {
                                        console.log(error1);
